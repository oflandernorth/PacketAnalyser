...
- [ ] **Implement `read_cap()` function** – allow analysis of existing PCAP files (using `scapy`’s `rdpcap`).
- [ ] **Time-series reporting** – add queries to show "Top 10 most persistent suspicious IPs over last 24 hours" and "IPs that changed risk level."
- [ ] **Human-readable report** – after capture, print a summary to console and optionally export to CSV/JSON.
- [ ] **Improve risk scoring** – make weights configurable in `config.yaml` (e.g., `risk_weights: {geoip: 5, abuseipdb: 10, port_scan: 20, volume: 5}`).
